...
imml_ptdf = @benchmark begin SCOPF.calc_ptdf!($ϕ, $X, $pf.X, $pf.B, $pf.DA, $cont1, $c1); LinearAlgebra.mul!($flow3, $ϕ, $Pᵢ); end # IMML ptdf
inv_ptdf = @benchmark begin SCOPF.calc_ptdf!($ϕ, $K, $pf.DA, $pf.B, $cont1, $c1, $pf.slack); LinearAlgebra.mul!($flow4, $ϕ, $Pᵢ); end # inverse with ptdf_pc
...
imml_ptdf = @benchmark begin SCOPF.calc_ptdf!($ϕ, $X, $pf.X, $pf.B, $pf.DA, $cont1, $c1); LinearAlgebra.mul!($flow3, $ϕ, ($Pᵢ .+ $ΔPc)); end # IMML ptdf
inv_ptdf = @benchmark begin SCOPF.calc_ptdf!($ϕ, $K, $pf.DA, $pf.B, $cont1, $c1, $pf.slack); LinearAlgebra.mul!($flow4, $ϕ, ($Pᵢ .+ $ΔPc)); end # inverse with ptdf
println("         IMML flow; IMML thet; inv theta; imml ptdf;  inv ptdf")
@printf("Min:    %9.0f; %9.0f; %9.0f; %9.0f; %9.0f\n", minimum(immlF).time, minimum(imml_theta).time, minimum(inv_theta).time, minimum(imml_ptdf).time, minimum(inv_ptdf).time)
@printf("Median: %9.0f; %9.0f; %9.0f; %9.0f; %9.0f\n", median(immlF).time, median(imml_theta).time, median(inv_theta).time, median(imml_ptdf).time, median(inv_ptdf).time)
...
inv_ptdf = @benchmark begin SCOPF.calc_ptdf!($ϕ, $pf.DA, $pf.B, $cont2, $c2, $pf.slack, $islands[island], $island_b); LinearAlgebra.mul!($flow3, ϕ, ($Pᵢ .+ $ΔPc)); end # inverse with ptdf
imml_theta = @benchmark SCOPF.calc_Pline!($flow4, $θ, $pf, $cont2, $c2, Pᵢ=($Pᵢ .+ $ΔPc), nodes=$islands[island], branches=$island_b) # IMML theta
imml_ptdf = @benchmark begin SCOPF.calc_ptdf!($ϕ, $X, $pf.X, $pf.B, $pf.DA, $cont2, $c2); LinearAlgebra.mul!($flow5, ϕ, ($Pᵢ .+ $ΔPc)); end # IMML ptdf
# hack = @benchmark begin SCOPF.calc_ptdf!($ϕ, $pf.ϕ, $islands[island], $island_b); LinearAlgebra.mul!($flow6, ϕ, ($Pᵢ .+ $ΔPc)); end
...
inv_ptdfvec = @benchmark SCOPF.calc_ptdf_vec!($pf.vn_tmp, $K, $pf.DA, $pf.B, $cont1, $c1, $pf.slack, 1) # inverse with ptdf-vec
imml_ptdf = @benchmark SCOPF.calc_ptdf!($ϕ, $X, $pf.X, $pf.B, $pf.DA, $cont1, $c1) # IMML ptdf
inv_ptdf = @benchmark SCOPF.calc_ptdf!($ϕ, $K, $pf.DA, $pf.B, $cont1, $c1, $pf.slack) # inverse with ptdf_pc

inv_ptdfvec_i = @benchmark SCOPF.calc_ptdf_vec!($pf.vn_tmp, $pf.DA, $pf.B, $cont2, $c2, $pf.slack, $islands[island], $island_b, 1) # inverse with ptdf-vec
inv_ptdf_i = @benchmark SCOPF.calc_ptdf!($ϕ, $pf.DA, $pf.B, $cont2, $c2, $pf.slack, $islands[island], $island_b) # inverse with ptdf
imml_ptdf_i = @benchmark SCOPF.calc_ptdf!($ϕ, $X, $pf.X, $pf.B, $pf.DA, $cont2, $c2) # IMML ptdf
