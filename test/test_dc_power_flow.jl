...
@test flow2 ≈ flow3
SCOPF.get_isf!(ϕ, K, pf.DA, pf.B, cont, c1, pf.slack); LinearAlgebra.mul!(flow4, ϕ, Pᵢ); # inverse with ptdf
@test flow3 ≈ flow4
SCOPF.calc_Pline!(flow5, θ, pf.X, pf.B, pf.DA, pf.θ, cont, c1) # IMML theta
@test flow4 ≈ flow5 
...
θ₂ = pf.X * (Pᵢ .+ ΔPc)
F₂ = pf.DA * θ₂
SCOPF.calculate_line_flows!(flow1, F₂, pf.ϕ, pf.B, pf.DA, pf.X, θ₂, cont[1], cont[2], c1) # IMML flow
...
@test flow2 ≈ flow3
SCOPF.get_isf!(ϕ, K, pf.DA, pf.B, cont, c1, pf.slack); LinearAlgebra.mul!(flow4, ϕ, (Pᵢ .+ ΔPc)); # inverse with ptdf
@test flow3 ≈ flow4
...
SCOPF.calculate_line_flows!(flow1, pf, cont, c2, Pᵢ=(Pᵢ .+ ΔPc), nodes=islands[island], branches=island_b) # IMML flow
...
@test flow2 ≈ flow3 
SCOPF.calc_Pline!(flow4, θ, pf.X, pf.B, pf.DA, θ₂, cont, c2) # IMML theta
@test flow3 ≈ flow4
SCOPF.get_isf!(ϕ, X, pf.X, pf.B, pf.DA, cont, c2); LinearAlgebra.mul!(flow5, ϕ, (Pᵢ .+ ΔPc)); # IMML ptdf
@test flow4 ≈ flow5 
...
# imml_theta = @benchmark SCOPF.calc_Pline!($flow5, $θ, $pf.X, $pf.B, $pf.DA, $pf.θ, $cont, $c1)
...
# ΔPc = rand(length(Pᵢ))
# immlF = @benchmark SCOPF.calculate_line_flows!(flow1, pf, cont, c1, Pᵢ=(Pᵢ .+ ΔPc)) # IMML flow
...
# cont = bx[c2]
# islands, island, island_b = SCOPF.handle_islands(pf.B, pf.DA, cont, c2, pf.slack)
# immlF = @benchmark SCOPF.calculate_line_flows!(flow1, pf, cont, c2, Pᵢ=(Pᵢ .+ ΔPc), nodes=islands[island], branches=island_b) # IMML flow
...
# imml_theta = @benchmark begin θ₂ = SCOPF.run_pf($pf.K, ($Pᵢ .+ $ΔPc), $pf.slack); SCOPF.calc_Pline!($flow4, $θ, $pf.X, $pf.B, $pf.DA, θ₂, $cont, $c2); end # IMML theta
# imml_ptdf = @benchmark begin SCOPF.get_isf!($ϕ, $X, $pf.X, $pf.B, $pf.DA, $cont, $c2); LinearAlgebra.mul!($flow5, ϕ, ($Pᵢ .+ $ΔPc)); end # IMML ptdf
