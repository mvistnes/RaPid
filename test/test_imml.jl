...
@test flow1 ≈ flow2
SCOPF.calc_ptdf!(ϕ, X, pf.X, pf.B, pf.DA, cont1, c1); LinearAlgebra.mul!(flow3, ϕ, Pᵢ); # IMML ptdf
@test flow2 ≈ flow3
SCOPF.calc_ptdf!(ϕ, K, pf.DA, pf.B, cont1, c1, pf.slack); LinearAlgebra.mul!(flow4, ϕ, Pᵢ); # inverse with ptdf
@test flow3 ≈ flow4
...
@test flow2 ≈ flow3
SCOPF.calc_ptdf!(ϕ, X, pf.X, pf.B, pf.DA, cont1, c1); LinearAlgebra.mul!(flow4, ϕ, (Pᵢ .+ ΔPc)); # IMML ptdf
@test flow3 ≈ flow4
SCOPF.calc_ptdf!(ϕ, K, pf.DA, pf.B, cont1, c1, pf.slack); LinearAlgebra.mul!(flow5, ϕ, (Pᵢ .+ ΔPc)); # inverse with ptdf
@test flow4 ≈ flow5
...
@test flow1 ≈ flow2
SCOPF.calc_ptdf!(ϕ, pf.DA, pf.B, cont2, c2, pf.slack, islands[island], island_b); LinearAlgebra.mul!(flow3, ϕ, (Pᵢ .+ ΔPc)); # inverse with ptdf
@test flow2 ≈ flow3 
SCOPF.calc_Pline!(flow4, θ, pf, cont2, c2, Pᵢ=(Pᵢ .+ ΔPc), nodes=islands[island], branches=island_b) # IMML theta
@test flow3 ≈ flow4
SCOPF.calc_ptdf!(ϕ, X, pf.X, pf.B, pf.DA, cont2, c2); LinearAlgebra.mul!(flow5, ϕ, (Pᵢ .+ ΔPc)); # IMML ptdf
@test flow4 ≈ flow5 
...
SCOPF.calculate_line_flows!(flow2, θ, B, pf.DA, pf.B, (Pᵢ .+ ΔPc), cont1, c1, pf.slack) # inverse with theta
@test flow1 ≈ flow2
SCOPF.calc_ptdf!(ϕ, X, pf.X, pf.B, pf.DA, cont1, c1); LinearAlgebra.mul!(flow3, ϕ, (Pᵢ .+ ΔPc)); # IMML ptdf
@test flow2 ≈ flow3
SCOPF.calc_ptdf!(ϕ, pf.K, pf.DA, pf.B, cont1, c1, pf.slack); LinearAlgebra.mul!(flow4, ϕ, (Pᵢ .+ ΔPc)); # inverse with ptdf
@test flow3 ≈ flow4
