...
function add_to!(vec::AbstractVector, varref::AbstractVector{VariableRef}, idx::Dict, 
    list::AbstractVector, func::Function
)
...
function print_variabels(list::AbstractVector, varref::AbstractVector{VariableRef})
    for (i, x) in enumerate(list)
        @printf(" %.3f, %s\n", JuMP.value(varref[i]), x.name)
    end
end

function print_corrective_values(list::AbstractVector{<:Component}, varref::AbstractVector{VariableRef}, 
    symb::Symbol
)
    for (i, x) in enumerate(list)
...
function print_contingency_results(opf::OPFsystem, Pc, Pcc, Pccx=Dict())
    if !isempty(Pc)
        for (i, c) in enumerate(opf.contingencies)
            ...
            if x != 0
                ...
                print_corrective_values(opf.ctrl_generation, x.pgc, :pgc)
    ...
    if !isempty(Pcc)
        for (i, c) in enumerate(opf.contingencies)
            ...
            if x != 0
                println("Contingency ", c.name)
                print_corrective_values(opf.ctrl_generation, x.pgu, :pgu)
                print_corrective_values(opf.ctrl_generation, x.pgd, :pgd)
...
function print_contingency_P(opf::OPFsystem, Pc, Pcc, Pccx=Dict())
    ...
    for (c, cont) in enumerate(opf.contingencies)
        ...
        x = get(Pc, c, 0)
        if x != 0
            add_to!(P, x.pgc, idx, opf.ctrl_generation, -)
