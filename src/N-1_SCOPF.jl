...
function scopf(opfm::OPFmodel; 
    ...
    add_c_branch!(opfm)
    add_obj!(opfm)
    add_lim_nonctrl_shed!(opfm, max_shed=max_shed, max_curtail=max_curtail)
    if unit_commit
...
function p_scopf(opfm::OPFmodel; 
    ...
    add_obj_cont!(opfm)
    add_lim_nonctrl_shed_cont!(opfm, islands, max_shed=max_shed, max_curtail=max_curtail)
    if circuit_breakers
        add_circuit_breakers_cont!(opfm, short_term_limit_multi)
    else
        add_c_branch_cont!(opfm, short_term_limit_multi)
    end
    if unit_commit
        add_unit_commit!(opfm)
    end
...
function pc_scopf(opfm::OPFmodel; 
    ...
    add_lim_nonctrl_shed_ccont!(opfm, islands, max_shed=max_shed, max_curtail=max_curtail)
    if circuit_breakers
        add_circuit_breakers_ccont!(opfm, short_term_limit_multi)
    else
        add_c_branch_ccont!(opfm, short_term_limit_multi)
    end
...
function add_obj_cont!(opfm::OPFmodel)
    ...
    @objective(opfm.mod, Min, objective_function(opfm.mod) + 
            sum(opfm.voll[d] * sum(opfm.prob[c] * opfm.mod[:lsc][d,c] for c in opfm.contingencies)
...
function add_obj_ccont!(opfm::OPFmodel, ramp_mult = 10)
    add_obj_cont!(opfm)
    @objective(opfm.mod, Min, objective_function(opfm.mod) + 
            sum(opfm.cost[g] * ramp_mult * sum(opfm.prob[c] * (opfm.mod[:pgu][g,c] #=+ opfm.mod[:pgd][g,c]*0.1=#) 
...
function add_c_bus!(opfm::OPFmodel, slack::Tuple{Integer, Bus}, listGen, listD, listRe)
    ...
    @constraint(opfm.mod, inj_p[n = get_name.(get_nodes(opfm.sys))], ctrl[n] .- 
            ...
            sum((-get_active_power(d) + opfm.mod[:ls0][get_name(d)] for d in listRe[n]), init = 0.0)
        )
...
function add_c_bus_cont!(opfm::OPFmodel, islands::Vector, slack::Tuple{Integer, Bus}, listGen, listD, listRe)
    ...
    for (i_c,c) in zip(islands, opfm.contingencies)
        ...
        for n in get_name.(get_sorted_nodes(opfm.sys))[i_c]
            @constraint(opfm.mod, opfm.mod[:ctrl][n] .- 
                ...
                sum((-get_active_power(d) + opfm.mod[:lsc][get_name(d),c] for d in listRe[n]), init = 0.0)
            )
        end
    end
...
function add_c_bus_ccont!(opfm::OPFmodel, islands::Vector, slack::Tuple{Integer, Bus}, listGen, listD, listRe)
    ...
    for (i_c,c) in zip(islands, opfm.contingencies)
        ...
        for n in get_name.(get_sorted_nodes(opfm.sys))[i_c]
            @constraint(opfm.mod,
                    ...
                    sum(-get_active_power(d) + opfm.mod[:lscc][get_name(d),c] for d in listRe[n])
                )
        end
    end
...
add_c_bus!(opfm::OPFmodel) = add_c_bus!(opfm, find_slack(opfm.sys), 
    make_list(opfm, get_ctrl_generation), make_list(opfm, get_demands), make_list(opfm, get_renewables))
add_c_bus_cont!(opfm::OPFmodel, islands::Vector) = add_c_bus_cont!(opfm, islands, find_slack(opfm.sys), 
    make_list(opfm, get_ctrl_generation), make_list(opfm, get_demands), make_list(opfm, get_renewables))
add_c_bus_ccont!(opfm::OPFmodel, islands::Vector) = add_c_bus_ccont!(opfm, islands, find_slack(opfm.sys), 
    make_list(opfm, get_ctrl_generation), make_list(opfm, get_demands), make_list(opfm, get_renewables))
...
function add_c_branch_ccont!(
    ...
    add_c_branch_cont!(opfm, x, branch_rating, short_term_limit_multi)
...
add_c_branch!(opfm::OPFmodel) = 
    add_c_branch!(opfm, make_named_array(get_x, get_branches(opfm.sys)), make_named_array(get_rate, get_branches(opfm.sys)))
add_c_branch_cont!(opfm::OPFmodel, short_term_limit_multi::Float64 = 1.0) = 
    add_c_branch_cont!(opfm, make_named_array(get_x, get_branches(opfm.sys)), make_named_array(get_rate, get_branches(opfm.sys)), short_term_limit_multi)
...
function add_lim_load_shed!(opfm::OPFmodel, listPd, max_shed::Float64 = 1.0)
...
function add_lim_load_shed_cont!(opfm::OPFmodel, islands::Vector, listPd, listD, max_shed::Float64 = 1.0)
    for (i_c,c) in zip(islands, opfm.contingencies)
        isempty(i_c) && continue
...
function add_lim_load_shed_ccont!(opfm::OPFmodel, islands::Vector, listPd, listD, max_shed::Float64 = 1.0)
    for (i_c,c) in zip(islands, opfm.contingencies)
        isempty(i_c) && continue
        for (i,n) in enumerate(get_name.(get_sorted_nodes(opfm.sys)))
            if i ∈ i_c
...
function add_lim_renewable_shed!(opfm::OPFmodel, listPr, max_curtail::Float64 = 1.0)
    @constraint(opfm.mod, renew_shed[d = get_name.(get_renewables(opfm.sys))], 
            opfm.mod[:ls0][d] <= listPr[d] * max_curtail
        )
    @info "Restriction of renewable shedding: Base case"
end
function add_lim_renewable_shed_cont!(opfm::OPFmodel, islands::Vector, listPr, listD, max_curtail::Float64 = 1.0)
    for (i_c,c) in zip(islands, opfm.contingencies)
        isempty(i_c) && continue
        for (i,n) in enumerate(get_name.(get_sorted_nodes(opfm.sys)))
            if i ∈ i_c
                @constraint(opfm.mod, renew_shed_cont[d = get_name.(listD[n])], opfm.mod[:lsc][d,c] <= listPr[d] * max_curtail)
            else
                @constraint(opfm.mod, renew_shed_cont[d = get_name.(listD[n])], opfm.mod[:lsc][d,c] == listPr[d])
            end
        end
    end
    @info "- After contingency, before corrective actions"
end
function add_lim_renewable_shed_ccont!(opfm::OPFmodel, islands::Vector, listPr, listD, max_curtail::Float64 = 1.0)
    for (i_c,c) in zip(islands, opfm.contingencies)
        isempty(i_c) && continue
        for (i,n) in enumerate(get_name.(get_sorted_nodes(opfm.sys)))
            if i ∈ i_c
                @constraint(opfm.mod, renew_shed_ccont[d = get_name.(listD[n])], opfm.mod[:lscc][d,c] <= listPr[d] * max_curtail)
            else
                @constraint(opfm.mod, renew_shed_ccont[d = get_name.(listD[n])], opfm.mod[:lscc][d,c] == listPr[d])
            end
        end
    end
    @info "- After contingency and corrective actions"
...
function add_lim_nonctrl_shed!(opfm::OPFmodel, listPd = make_named_array(get_active_power, get_demands(opfm.sys)),
        listPr = make_named_array(get_active_power, get_renewables(opfm.sys)); max_shed::Float64 = 1.0, max_curtail::Float64 = 1.0)
    add_lim_load_shed!(opfm, listPd, max_shed)
    !isempty(listPr) && add_lim_renewable_shed!(opfm, listPr, max_curtail)
end
function add_lim_nonctrl_shed_cont!(opfm::OPFmodel, islands::Vector, listPd = make_named_array(get_active_power, get_demands(opfm.sys)),
        listPr = make_named_array(get_active_power, get_renewables(opfm.sys)); max_shed::Float64 = 1.0, max_curtail::Float64 = 1.0) 
    add_lim_nonctrl_shed!(opfm, listPd, listPr, max_shed=max_shed, max_curtail=max_curtail)
    listD = make_list(opfm, get_demands)
    !isempty(listPr) && add_lim_renewable_shed_cont!(opfm, islands, listPr, listD, max_curtail)
    add_lim_load_shed_cont!(opfm, islands, listPd, listD, max_shed)
end
function add_lim_nonctrl_shed_ccont!(opfm::OPFmodel, islands::Vector, listPd = make_named_array(get_active_power, get_demands(opfm.sys)),
        listPr = make_named_array(get_active_power, get_renewables(opfm.sys)); max_shed::Float64 = 1.0, max_curtail::Float64 = 1.0) 
    add_lim_nonctrl_shed_cont!(opfm, islands, listPd, listPr, max_shed=max_shed, max_curtail=max_curtail)
    listD = make_list(opfm, get_demands)
    !isempty(listPr) && add_lim_renewable_shed_ccont!(opfm, islands, listPr, listD, max_curtail)
    add_lim_load_shed_ccont!(opfm, islands, listPd, listD, max_shed)
end
